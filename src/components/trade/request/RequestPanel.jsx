...
import { CARD, INPUT_NUM, INPUT_TEXT, LABEL, BTN, BTN_PROMPT } from '../shared/tradeStyles';
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="space-y-1">
          ...
          {toToken && (
            <div className="flex items-center gap-1.5 mt-1.5">
              {DOLLAR_PRESETS.map(usd => {
                ...
                return (
                  <button key={usd} type="button" className="text-[10px] text-primary bg-primary/10 border border-primary/20 hover:bg-primary/20 px-2 py-0.5 rounded-full transition-colors"
                    onClick={() => {
              ...
              {toAmount && (
                <button type="button" className="text-[10px] text-red-400 bg-red-900/20 border border-red-800/30 hover:bg-red-900/30 px-2 py-0.5 rounded-full transition-colors"
                  onClick={() => settoAmount('')}>
                  Clear
                </button>
              )}
        ...
        <button type="submit" className={canSubmit ? BTN : BTN_PROMPT} disabled={submitting}>
